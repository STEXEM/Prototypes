...
////////////////////////////          SETS            //////////////////////////////////
 ...
 // Temporal Indixes
   ...
   p1(p)         first period
   pn(p)         last period
   IndexRP (p,p) index relating representative periods with periods
   q             index for enumarating tranmission candidates /q1*q64/
   qqq(q)        dynamic set to solve a subset of q           /q1/
 ...
 // Generators Indixes
   ...
   sr(g)         Solar
   cp            companies
   gcp(g,cp)     univocal correspondace from generator to companies
...
////////////////////////////       PARAMETERS         //////////////////////////////////
 ...
 // Generation parameters
   ...
   pEFOR          (      g    )   EFOR                                [p.u.]
   pWindGen       ( p,   g    )   maximun wind generation per period  [GW]
   pMaxWindGen    (      g,d  )   maximun wind generation per node    [GW]
   pSolarGen      ( p,   g    )   solar generation                    [GW]
   pSolarGenNode  ( p,   g,d  )   maximun solar generation per period [GW]
   pMaxSolarGen   (      g,d  )   maximun solar generation per node   [GW]
   ...
   pReservoir     (     g     )   status of hydro unit 1= reservoir 0 =runoftheriver
...
////////////////////////////  EQUATION DECLARATION    //////////////////////////////////
 ...
 // Generation Constraints
   ...
   BudgetThermal   (y,   g, d)  max wind investment                 [GW]
   BudgetHydro     (y,   g, d)  max wind investment                 [GW]
   BudgetStorage   (y,   g, d)  max wind investment                 [GW]
 // Network Constraints
...
////////////////////////////   EQUATION DEFINITION    //////////////////////////////////
 ...
 //**************** EQUALITY CONSTRAINT / NO COMPLEMENTARITY **************************
  ...
  eRSRVH_RP1_C_CAN (y,ps(p),gad(hs,d)) $[[ORD(p) <= CARD(p) ] and pReservoir(hs) ]..
 ...
 //*********************       PRIMAL CONSTRAINTS       ********************************
  // Conventioanl and wind constraints
     eMaxProd        (y,pa(p),ged(g,d) )$(not wn(g) and not sr(g)) ..- vProduct(y,p,g,d)           =g= -  pMaxProd(g,d)+ vNewGen (y, g,d) ;//$t(g)-pMaxProd(g,d)$ (hs(g) and pReservoir(g)) -pInflows(p,g)$ (hs(g) and not (pReservoir(g)) )     ; // b_cRo_up    (y,p,g  )
     ...
     BudgetSolar     (y,      gcd(sr,d)) ..- vNewGen (y, sr,d)                       =g= -  4                                                                             ;
     BudgetWind      (y,      gcd(wn,d)) ..- vNewGen (y, wn,d)                       =g= -  4                                                                             ;
     BudgetThermal   (y,      gcd(t,d)) ..- vNewGen (y, t,d)                         =g= -  4                                                                             ;
     BudgetHydro     (y,      gcd(hs,d)) ..- vNewGen (y, hs,d)                       =g= -  4                                                                             ;
     BudgetStorage   (y,      gcd(hf,d)) ..- vNewGen (y, hf,d)                       =g= -  4                                                                             ;
  ...
  // storage constraints
     ...
     eMaxCons_New    (y,pa(p),gcd(h,d) ) ..- vConsump(y,p,h,d)                                            =g= -  vNewGen (y, h,d)*pMaxCons(h)*(2-pEffic(h))                         ; // b_KappN_up  (y,p,h,d)
  ...
  // Network Constraints
     ...
     eFlowNetEx      (y,pa(p),le(di,df))$(pX(di,df)).. vFlow(y,p,  di,df)                      =e=   [vTheta(y,p,di) - vTheta(y,p,df)] * pSbase / pX(di,df)                           ; // b_phi (y,p,s,d,d)
     eFlowNetN1      (y,pa(p),lc(di,df))$(pX(di,df))..  vFlow(y,p,  di,df) / (1e+3/1e+3)*pTTC(di,df) =g=   [vTheta(y,p,di) - vTheta(y,p,df)] * pSbase / pX(di,df) / (1e+3/1e+3)*pTTC(di,df)
                                                                                                                                      - 1 + vNewLine(y,di,df)             ; // b_tau_lo (y,p,s,d,d)
     eFlowNetN2      (y,pa(p),lc(di,df))$(pX(di,df)) ..- vFlow(y,p,  di,df) / (1e+3/1e+3)*pTTC(di,df) =g= -([vTheta(y,p,di) - vTheta(y,p,df)] * pSbase / pX(di,df) / (1e+3/1e+3)*pTTC(di,df)
...
//////////////////////////// DYNAMIC SETS AND SCALING //////////////////////////////////
 // Activating period and generation sets
     ...
     wn(g) $[pWindGen   ('h5497',     g)                              ] = yes ;
     sr(g) $[pSolarGen  ('h5497',     g)                              ] = yes ;
 ...
 // Scaling parameters
    ...
    pDemandNode   (y,p,d)   = sum(ct, pDist(d,ct)*pDemandCountry(p,ct)  * pCumDemIncr(y))* 1e-3; // MW to GW
    display pDemandNode;
 ...
 // Storage Parameters
    ...
    pReservoir   (h)    = pStorageGen(h,'Reservoir'    )        ;
    pEprMin      (h)    = 0                                     ;
    pInflows    (p,h)   = pInflow     (p,h              )* 1e-3 ;
...
////////////////////////////      MODEL SOLVING       //////////////////////////////////
 ...
 //////////////////////////// Solving Market as a QCP //////////////////////////////////////
  ...
  //pBenefits  (cp )   =        + sum((y,rpp(rp,pa(p)),gad(t,d),gcp(t,cp)),(  vProduct.l(y,p,t,d)*( eBalance_C_can.m (y,p,d)- pSlopeVarCost(t)*pWeight_rp(RP)   ) ))   //
  //                            + sum((y,rpp(rp,pa(p)),gad(h,d),gcp(h,cp)),(  eBalance_C_can.m (y,p,d)*( vProduct.l(y,p,h,d)- vConsump.l(y,p,h,d)) ))
  //                            - sum((y,              gad(g,d),gcp(g,cp)),   vNewGen.l   (y,   g,d)*pGenInvCost (g))
  //                            - sum[(y,rpp(rp,pa(p)),gad(h,d),gcp(h,cp)), pWeight_rp(rp)* pHydroCost*vProduct.l(y,p,h,d)]
  //                            - sum[(y,rpp(rp,pa(p)),gad(h,d),gcp(h,cp)), pWeight_rp(rp)* pConsumCost*vConsump.l(y,p,h,d)]
  //                             + sum((gad(g,d),gcp(g,cp)), eps);
  //         ;
   ...
   pCongestionRents =
           ...
           + sum[(y,lc),      (card(y)-ord(y)+1)*pFixedCost (lc)*[vNewLine.l(y,lc)        - vNewLine.l  (y-1,lc)]]             ;
            ...
            $$offtext
        ;
   //vProduct.up  (y,pa(p),s, gad(h,d)) =  pProduct_FX(y,p,s,h,d)          ;
   // b_cLambda.lo (y,pa(p),        d) = sum(rpp(rp,p), eBalance_C_can.m (y,p,     d   )/pWeight_rp(rp));
   //b_phi_up.fx  (y,pa(p),s,le(di,df)) = eFlowExisting2.m  (y,p,s,di,df);
   //vReserve.l  (y,ps(p),gad(hs,d) )    =   pReserve_FX(y,p,hs,d) ;

  // pObjFunct (f,'4') =  pTotalCost2 + EPS              ;
  // pObjFunct (f,'5') =  pExtendedWelfare + EPS          ;

  // Y_cRo_up.l   (y,pa(p),s,g,d ) $((Y_cRo_up.l   (y,p,s,g,d ) < 1) and  (Y_cRo_up.l   (y,p,s,g,d )  >  0))= 1;
   //Y_cRo_up.l   (y,pa(p),s,ged(g,d)) = 1;
  display pTotalCost1, pExtendedWelfare, pWindSpillage;
 ...
 //////////////////////////// Final Printing          /////////////////////////////////////
   ...
   pObjFunct (q,'1') =  +sum[(y,gcd(g,d)),(card(y)-ord(y)+1)*pGenInvCost (g)*[vNewGen.l(y, g,d) - vNewGen.l(y-1, g,d)]*(pMaxProd(g,d)+pMaxWindGen (g,d)$wn(g)+pMaxSolarGen (g,d)$sr(g))]
                                                                                                                                                  ;
   pObjFunct (q,'2') =  +sum[(y,lc),      (card(y)-ord(y)+1)*pFixedCost (lc)*[vNewLine.l(y,lc)      - vNewLine.l   (y-1,lc)  ]] + EPS             ;
   pObjFunct (q,'3') =  pTotalCost1     + EPS             ;
   pObjFunct (q,'4') =  +sum[(y,gcd(g,d)),(card(y)-ord(y)+1)*pGenInvCost (g)*[vNewGen.l(y, g,d) - vNewGen.l(y-1, g,d)]*(pMaxProd(g,d)+pMaxWindGen (g,d)$wn(g))+pMaxSolarGen (g,d)$sr(g)] + EPS
                        +sum[(y,lc),      (card(y)-ord(y)+1)*pFixedCost (lc)*[vNewLine.l(y,lc)        - vNewLine.l     (y-1,lc)  ]]
                        + pTotalCost1     + EPS             ;
